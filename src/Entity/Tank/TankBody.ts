...
export default class TankBody extends LivingEntity implements BarrelBase {
    ...
    public onKill(entity: LivingEntity) {
        ...
        if ((process['ALL_ENTITIES'] || entity instanceof Square)) {
                if (this._currentTank === Tank.Caster && this.barrels.length && this.barrels[0].droneCount < (15 + this.cameraEntity.camera.values.statLevels.values[Stat.Reload] * 1.5)){
            ...
            if (this._currentTank === Tank.Maleficitor && this.barrels.length && this.barrels[0].droneCount < (15 + this.cameraEntity.camera.values.statLevels.values[Stat.Reload] * 1.75)){
                // No destroy it on the next tick to make it look more like the way diep does it.
                entity.destroy(true);
                if (entity.deletionAnimation) {
                    entity.deletionAnimation.frame = 0;
            ...
            if (this._currentTank === Tank.Necromancer && this.barrels.length && this.barrels[0].droneCount < (22 + this.cameraEntity.camera.values.statLevels.values[Stat.Reload] * 2)){
                // No destroy it on the next tick to make it look more like the way diep does it.
                entity.destroy(true);
                if (entity.deletionAnimation) {
                    entity.deletionAnimation.frame = 0;
