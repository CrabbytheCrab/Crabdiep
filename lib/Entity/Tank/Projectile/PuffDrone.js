...
class Drone extends Bullet_1.default {
    constructor(barrel, tank, tankDefinition, shootAngle, canrepel = true) {
    ...
    tick(tick) {
        if (this.canrepel) {
            ...
            const usingAI = !this.canControlDrones || this.tank.inputs.deleted || (!this.tank.inputs.attemptingShot());
            const inputs = !usingAI ? this.tank.inputs : this.ai.inputs;
            if (usingAI && this.ai.state === 0) {
                const delta = {
                    x: this.positionData.values.x - this.tank.positionData.values.x,
