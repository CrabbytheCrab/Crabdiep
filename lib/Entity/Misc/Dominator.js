...
class Dominator extends TankBody_1.default {
    ...
    onDeath(killer) {
        ...
        if (this.relationsData.values.team === this.game.arena && killer instanceof TankBody_1.default) {
            ...
            this.game.broadcast().u8(3).stringNT(message).u32(killerTeamIsATeam ? Enums_1.ColorsHexCode[killerTeam.teamData.values.teamColor] : 0x000000).float(10000).stringNT("").send();
            for (const client of this.game.clients) {
                if (client.camera?.teamData?.teamColor == this.teamData?.teamColor) {
                    client.notify("Press H to take control of the dominator", killerTeamIsATeam ? Enums_1.ColorsHexCode[killerTeam.teamData.values.teamColor] : 0x000000, 10000);
                }
            }
        }
        else {
            ...
            this.game.broadcast().u8(3).stringNT(message).u32(Enums_1.ColorsHexCode[this.game.arena.teamData.teamColor]).float(10000).stringNT("").send();
        }
