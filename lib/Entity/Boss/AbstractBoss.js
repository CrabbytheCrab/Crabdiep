...
var TargetShort;
(function (TargetShort) {
    TargetShort[TargetShort["None"] = -1] = "None";
    TargetShort[TargetShort["BottomRight"] = 0] = "BottomRight";
    TargetShort[TargetShort["TopRight"] = 1] = "TopRight";
    TargetShort[TargetShort["TopLeft"] = 2] = "TopLeft";
    TargetShort[TargetShort["BottomLeft"] = 3] = "BottomLeft";
})(TargetShort || (TargetShort = {}));
class BossMovementControl {
    constructor(boss) {
        this.target = Target.None;
        this.targetshort = TargetShort.None;
        this.boss = boss;
    ...
    moveBossShort() {
        if (this.boss.game.gamemode == "bossbash") {
            ...
            const target = this.targetshort === TargetShort.BottomRight ?
            ...
            const dist = target.x ** 2 + target.y ** 2;
            if (dist < 90000)
                this.targetshort = (this.targetshort + 1) % 4;
            else {
                const angle = Math.atan2(target.y, target.x);
                this.boss.inputs.movement.x = Math.cos(angle);
                this.boss.inputs.movement.y = Math.sin(angle);
