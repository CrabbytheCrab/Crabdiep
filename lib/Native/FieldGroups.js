...
exports.TeamGroup = exports.ScoreGroup = exports.StyleGroup = exports.PositionGroup = exports.CameraGroup = exports.CameraTable = exports.NameGroup = exports.ArenaGroup = exports.ScoreboardTable = exports.UnusedGroup = exports.HealthGroup = exports.PhysicsGroup = exports.BarrelGroup = exports.RelationsGroup = void 0;
/**
 * UNDOCUMENTED FILE.
 *
...
class RelationsGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.relations);
        this.entity = entity;
...
class BarrelGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.barrel);
        this.entity = entity;
    ...
    findUpdate() {
        ...
        if (this.state.shooting)
            out.push("shooting");
        if (this.state.reloadTime)
            out.push("reloadTime");
        if (this.state.trapezoidalDir)
            out.push("trapezoidalDir");
        return out;
    }
    wipe() {
    ...
    set shooting(shooting) {
        if (shooting == this.values.shooting)
            return;
    ...
    set reloadTime(reloadTime) {
        if (reloadTime == this.values.reloadTime)
            return;
    ...
    set trapezoidalDir(trapezoidalDir) {
        if (trapezoidalDir == this.values.trapezoidalDir)
            return;
...
class PhysicsGroup {
    constructor(entity) {
        ...
        this.queryId = -1;
        entity.fieldGroups.push(Entity_1.FieldGroupID.physics);
        this.entity = entity;
    ...
    findUpdate() {
        const out = [];
        if (this.state.objectFlags)
            out.push("objectFlags");
        if (this.state.sides)
            out.push("sides");
        if (this.state.size)
            out.push("size");
        if (this.state.width)
            out.push("width");
        if (this.state.absorbtionFactor)
            out.push("absorbtionFactor");
        if (this.state.pushFactor)
            out.push("pushFactor");
        return out;
    }
    wipe() {
    ...
    set objectFlags(objectFlags) {
        if (objectFlags == this.values.objectFlags)
            return;
    ...
    set absorbtionFactor(absorbtionFactor) {
        if (absorbtionFactor == this.values.absorbtionFactor)
            return;
    ...
    set pushFactor(pushFactor) {
        if (pushFactor == this.values.pushFactor)
            return;
...
class HealthGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.health);
        this.entity = entity;
    ...
    set healthbar(healthbar) {
        if (healthbar == this.values.healthbar)
            return;
    ...
    set maxHealth(maxHealth) {
        if (maxHealth == this.values.maxHealth)
            return;
...
class UnusedGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.unused);
        this.entity = entity;
        this.fields = ["unknown"];
        this.state = {
            unknown: 0
    ...
    set unknown(unknown) {
        if (unknown == this.values.unknown)
            return;
...
exports.UnusedGroup = UnusedGroup;
class ScoreboardTable {
    constructor(defaultValue, fieldName, owner) {
    ...
    findUpdate() {
        return this.state.reduce((out, v, i) => {
            if (v)
                out.push(i);
            return out;
        }, []);
...
class ArenaGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.arena);
        this.entity = entity;
        ...
        this.values = {
            ...
            scoreboardColors: new ScoreboardTable(Enums_1.Colors.ScoreboardBar, "scoreboardColors", this),
            scoreboardSuffixes: new ScoreboardTable("", "scoreboardSuffixes", this),
            scoreboardTanks: new ScoreboardTable(Enums_1.Tank.Basic, "scoreboardTanks", this),
            leaderX: 0,
            leaderY: 0,
            playersNeeded: 1,
            ticksUntilStart: 250
    ...
    findUpdate() {
        ...
        if (this.state.topY)
            out.push("topY");
        if (this.state.rightX)
            out.push("rightX");
        if (this.state.bottomY)
            out.push("bottomY");
        if (this.state.scoreboardAmount)
            out.push("scoreboardAmount");
        if (this.state.scoreboardNames)
            out.push("scoreboardNames");
        if (this.state.scoreboardScores)
            out.push("scoreboardScores");
        if (this.state.scoreboardColors)
            out.push("scoreboardColors");
        if (this.state.scoreboardSuffixes)
            out.push("scoreboardSuffixes");
        if (this.state.scoreboardTanks)
            out.push("scoreboardTanks");
        if (this.state.leaderX)
            out.push("leaderX");
        if (this.state.leaderY)
            out.push("leaderY");
        if (this.state.playersNeeded)
            out.push("playersNeeded");
        if (this.state.ticksUntilStart)
            out.push("ticksUntilStart");
        return out;
    }
    wipe() {
        this.state.GUI = 0;
    ...
    set bottomY(bottomY) {
        if (bottomY == this.values.bottomY)
            return;
    ...
    set scoreboardAmount(scoreboardAmount) {
        if (scoreboardAmount == this.values.scoreboardAmount)
            return;
    ...
    set leaderX(leaderX) {
        if (leaderX == this.values.leaderX)
            return;
    ...
    set leaderY(leaderY) {
        if (leaderY == this.values.leaderY)
            return;
    ...
    set playersNeeded(playersNeeded) {
        if (playersNeeded == this.values.playersNeeded)
            return;
    ...
    set ticksUntilStart(ticksUntilStart) {
        if (ticksUntilStart == this.values.ticksUntilStart)
            return;
...
class NameGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.name);
        this.entity = entity;
    ...
    set nametag(nametag) {
        if (nametag == this.values.nametag)
            return;
...
class CameraGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.camera);
        this.entity = entity;
    ...
    findUpdate() {
        ...
        if (this.state.tank)
            out.push("tank");
        if (this.state.levelbarProgress)
            out.push("levelbarProgress");
        if (this.state.levelbarMax)
            out.push("levelbarMax");
        if (this.state.statsAvailable)
            out.push("statsAvailable");
        if (this.state.statNames)
            out.push("statNames");
        if (this.state.statLevels)
            out.push("statLevels");
        if (this.state.statLimits)
            out.push("statLimits");
        if (this.state.cameraX)
            out.push("cameraX");
        if (this.state.cameraY)
            out.push("cameraY");
        if (this.state.scorebar)
            out.push("scorebar");
        if (this.state.respawnLevel)
            out.push("respawnLevel");
        if (this.state.killedBy)
            out.push("killedBy");
        if (this.state.spawnTick)
            out.push("spawnTick");
        if (this.state.deathTick)
            out.push("deathTick");
        if (this.state.tankOverride)
            out.push("tankOverride");
        if (this.state.movementSpeed)
            out.push("movementSpeed");
        return out;
    }
    wipe() {
    ...
    set GUIunknown(GUIunknown) {
        if (GUIunknown == this.values.GUIunknown)
            return;
    ...
    set levelbarProgress(levelbarProgress) {
        if (levelbarProgress == this.values.levelbarProgress)
            return;
    ...
    set levelbarMax(levelbarMax) {
        if (levelbarMax == this.values.levelbarMax)
            return;
    ...
    set statsAvailable(statsAvailable) {
        if (statsAvailable == this.values.statsAvailable)
            return;
    ...
    set cameraX(cameraX) {
        if (cameraX == this.values.cameraX)
            return;
    ...
    set cameraY(cameraY) {
        if (cameraY == this.values.cameraY)
            return;
    ...
    set scorebar(scorebar) {
        if (scorebar == this.values.scorebar)
            return;
    ...
    set respawnLevel(respawnLevel) {
        if (respawnLevel == this.values.respawnLevel)
            return;
    ...
    set killedBy(killedBy) {
        if (killedBy == this.values.killedBy)
            return;
    ...
    set spawnTick(spawnTick) {
        if (spawnTick == this.values.spawnTick)
            return;
    ...
    set deathTick(deathTick) {
        if (deathTick == this.values.deathTick)
            return;
    ...
    set tankOverride(tankOverride) {
        if (tankOverride == this.values.tankOverride)
            return;
    ...
    set movementSpeed(movementSpeed) {
        if (movementSpeed == this.values.movementSpeed)
            return;
...
class PositionGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.position);
        this.entity = entity;
...
class StyleGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.style);
        this.entity = entity;
    ...
    findUpdate() {
        const out = [];
        if (this.state.styleFlags)
            out.push("styleFlags");
        if (this.state.color)
            out.push("color");
        if (this.state.borderThickness)
            out.push("borderThickness");
    ...
    set styleFlags(styleFlags) {
        if (styleFlags == this.values.styleFlags)
            return;
    ...
    set borderThickness(borderThickness) {
        if (borderThickness == this.values.borderThickness)
            return;
...
class ScoreGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.score);
        this.entity = entity;
        this.fields = ["score"];
        this.state = {
            score: 0
...
class TeamGroup {
    constructor(entity) {
        entity.fieldGroups.push(Entity_1.FieldGroupID.team);
        this.entity = entity;
        ...
        this.values = {
            teamColor: Enums_1.Colors.Border,
            mothershipX: 0,
            mothershipY: 0,
            mothership: 0
        };
    ...
    findUpdate() {
        ...
        if (this.state.teamColor)
            out.push("teamColor");
        if (this.state.mothershipX)
            out.push("mothershipX");
        if (this.state.mothershipY)
            out.push("mothershipY");
        if (this.state.mothership)
            out.push("mothership");
        return out;
    }
    wipe() {
    ...
    set teamColor(teamColor) {
        if (teamColor == this.values.teamColor)
            return;
    ...
    set mothershipX(mothershipX) {
        if (mothershipX == this.values.mothershipX)
            return;
    ...
    set mothershipY(mothershipY) {
        if (mothershipY == this.values.mothershipY)
            return;
    ...
    set mothership(mothership) {
        if (mothership == this.values.mothership)
            return;
